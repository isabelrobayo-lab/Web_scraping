...
const basePath = `${SITE_BASE}worlds/${WORLD_ID}`;

const MINIVERSE_API = (import.meta.env.VITE_MINIVERSE_API || 'http://localhost:4321').replace(/\/$/, '');
const MINIVERSE_WS = import.meta.env.VITE_MINIVERSE_WS || 'ws://localhost:4321/ws';

function charSprites(name: string): SpriteSheetConfig {
  return {
    sheets: {
      walk: `${SITE_BASE}universal_assets/citizens/${name}_walk.png`,
      actions: `${SITE_BASE}universal_assets/citizens/${name}_actions.png`,
    },
    animations: {
...
async function main() {
  ...
  const serverAgents: { agent: string; name: string }[] = await fetch(`${MINIVERSE_API}/api/agents`)
    .then(r => r.json())
    .then((d: any) => d.agents ?? [])
    .catch(() => []);
  ...
  // Hook para E2E: los nombres se renderizan en canvas, no como texto DOM
  (window as unknown as { __E2E_MINIVERSE_CITIZENS__?: number }).__E2E_MINIVERSE_CITIZENS__ =
    citizens.length;
